...
struct ContentView: View {
    ...
    /// Artifact cards — one per file this turn produced (generated images, written/saved documents),
    /// each with in-app Quick Look, Save As…, and Reveal. Uses the core's structured `artifactPaths`;
    /// for transcripts saved before that field existed, falls back to the legacy imagePath/docPath so
    /// older conversations still render their artifacts.
    @ViewBuilder private func artifactCards(_ msg: ChatMsg) -> some View {
        let raw = msg.artifactPaths.isEmpty
            ? [msg.imagePath, msg.docPath].compactMap { $0 }
            : msg.artifactPaths
        let paths = Artifact.normalize(raw)
        if !paths.isEmpty {
            VStack(alignment: .leading, spacing: 10) {
                ForEach(paths, id: \.self) { ArtifactCard(path: $0) }
            }
        }
    }
